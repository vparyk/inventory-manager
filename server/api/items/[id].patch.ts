...
import {
  InventoryItemBase,
  ItemsPatchApiResponse,
} from "~~/shared/types/inventory-item";

export default defineEventHandler(
  async (event): Promise<ItemsPatchApiResponse> => {
    ...
    const body = await readBody<{ quantity: number; lastUpdated: string }>(
      event
    );
    ...
    const result: InventoryItemBase = updateItem(id, body.quantity);
